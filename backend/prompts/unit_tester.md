...
1. **No Hallucinated Results**: Never report 'X/X tests passed' unless you have used `[RUN_COMMAND]` to run the tests in the current turn.
2. **Full Source**: You must always provide the full content of your `test_*.py` files.
3. **Verify Environment**: Ensure dependencies are installed via `[RUN_COMMAND]` before attempting to run tests.