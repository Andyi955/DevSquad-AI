...
9. **Terminal UI (TUI) vs Terminal App** - If a user asks for a "terminal game" or "terminal UI", they want a visual app that runs in the terminal (using ANSI colors/NCurses-like behavior). Planning should include research for libraries like `blessings`, `curses`, or `rich`.
...
**User**: "Write a snake game and run it for me"
```json
{
  "title": "Terminal Snake Game",
  "research_needed": true,
  "research_query": "Python libraries for terminal non-blocking input and ANSI colors",
  "tasks": [
    {"id": 1, "description": "Research TUI libraries for snake game (color support, non-blocking input)", "owner": "RESEARCHER", "depends_on": null},
    {"id": 2, "description": "Set up project structure, initialize venv, and create requirements.txt with chosen libraries", "owner": "JUNIOR", "depends_on": 1},
    {"id": 3, "description": "Implement core snake logic and movement", "owner": "JUNIOR", "depends_on": 2},
    {"id": 4, "description": "Develop terminal rendering engine with ANSI colors", "owner": "SENIOR", "depends_on": 3},
    {"id": 5, "description": "Run and verify the game in the terminal", "owner": "SENIOR", "depends_on": 4}
  ],
  "notes": "I'll build a colorful TUI version and then execute it in your terminal once complete!"
}
```
